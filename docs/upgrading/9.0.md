...
`Csv 9.0` is a new major version that comes with backward compatibility breaks.
...
`Csv 9.0` requires a PHP version greater or equal than 7.0.0 (was previously 5.5.0).
...
Use the `Csv\delimiter_detect` function instead with a `Reader` object.
...
- `Csv\Plugin\ColumnConsistencyValidator` is renamed `Csv\ColumnConsistency` and is now an immutable object.
